export const services = [
  {
    slug: "cad-cam-development",
    title: "CAD/CAM Development",
    features: [
      {
        id: 1,
        title: "Geometry Kernel",
        icon: "bi bi-cpu",
        contentTitle: "Geometry Kernel",
        points: [
          "Basic CAD/CAM Features",
          "Advanced Functionalities for FE Analysis",
          "Geometric Manipulation",
        ],
      },
      {
        id: 2,
        title: "Interfaces",
        icon: "bi bi-diagram-3",
        contentTitle: "Interfaces",
        points: [
          "Translators for Neutral Formats STEP, VRML, STL, IGES, DXF, Parasolid, Granite ACIS, JT Open etc.",
          "Plug-in Development for 3rd party software Pro/ENGINEER, SolidWorks, CATIA V5/V6, Autodesk Inventor, SpaceClaim",
        ],
      },
      {
        id: 3,
        title: "Integration Projects",
        icon: "bi bi-diagram-2",
        contentTitle: "Integration Projects",
        points: [
          "Integrating client software in 3rd party software like Pro/ENGINEER, SolidWorks, CATIA V5/V6, AI, SpaceClaim",
        ],
      },
      {
        id: 4,
        title: "Application Development",
        icon: "bi bi-app-indicator",
        contentTitle: "Application Development",
        content:
          "We use SDK's available with CAD software to customize and develop new tools and enhance the existing features as per the customer requirements.\n\nA software tool developed within the client's existing CAD Software and is responsible for providing extra features, utilities/ Value addition. (E.g. changing GUI, adding menus, or developing macros take several steps with a single button click).",
        highlight:
          "We have expertise with technologies and SDK’s of following CAD Software.",

        points: [
          "SolidWorks: Visual Basic .NET (VB.NET), Visual C++/CLI, Visual C# .NET, Visual C++ 6.0",
          "CATIA: CAA (Component Application Architecture)v5/V6, Macro Development, CATScript, VB Script, C++",
          "UG : C, C++, NX Open API",
          "AutoDesk: API, Inventor SDK, VBA, C++",
          "ProE : ProTool Kit, Pro/web.link, JLink",
        ],
      },
      {
        id: 5,
        title: "1D Multi Domain System Simulation",
        icon: "bi bi-eye",
        contentTitle: "Graphics and Visualization",
        points: [
          "Shaded Exterior and Edges",
          "Wireframe",
          "Body Transparency",
          "Rotation Modes",
        ],
      },
      {
        id: 6,
        title: ".NET Development",
        icon: "bi bi-code-slash",
        contentTitle: ".NET Development ",
        content:
          "VJCS .NET development team has solid experience in software programming using various Microsoft development environments (C#, ASP .NET, VB .NET, WinForms and others). We have experienced software development professionals who are ready to endow their knowledge to development and enhancement of solutions that we offer to our clients.\n\nWe offer end-to-end product lifecycle management, test-driven development, and insights into the possibilities and pitfalls of the process as keys to success. An eye for project architecture allows for Custom .NET Development to chart the appropriate course from the beginning.",

        highlight: "Custom .NET development services include:",

        points: [
          "Custom .NET web-based solution development",
          ".NET desktop applications migration",
          ".NET web applications migration",
          ".NET custom software development",
          "Legacy .NET applications updating, enhancement, and support",
          ".NET mobile applications development (based on .NET Compact Framework)",
          ".NET application development consulting",
        ],
      },
      {
        id: 7,
        title: "CLOUD & WEB APPLICATION DEVELOPMENT",
        icon: "bi bi-cloud",
        contentTitle: "Cloud & Web Application Development",
        points: [
          "Windows Azure (Virtual Machine, Services, Web Role, Worker Role, Storages, SQL Azure)",
          "C/C++/.NET/Java",
          ".NET Framework 4.0, C#.NET, ASP.NET, ADO.NET",
          "AJAX, Microsoft Enterprise Library",
          "Silverlight, WPF, LINQ and Entity Framework",
          "Web Services (WCF, ASP)",
          "SQL Server 2012/2008, MySQL",
        ],
      },
      {
        id: 8,
        title: "Technologies",
        icon: "bi bi-tools",
        contentTitle: "Technologies",
        points: [
          "Languages: C, C++, C#, Visual Basic, Java",
          "Graphic Libraries: Qt, Coin3D, OpenGL, OpenCascade, VTK, WebQL",
          "ACIS/Parasolid, Geometric Algorithms",
          "CATIA CAA V5/V6, Pro/TOOLKIT, SolidWorks API",
        ],
      },
      {
        id: 7,
        title: "NC Programming Services",
        icon: "bi bi-cpu",
        contentTitle: "NC Programming Services",
        content:
          "VJ Core soft offers companies a way to extend their NC Programming operations to help crunch through large projects, and start the chips flying. Our team has helped major manufacturers reach tight deadlines and expand their capabilities to bring in more work.\n\nWe have vast experience at in CNC programming for complex geometries and profiles. Our manufacturing engineering capabilities help us understand and optimize a variety of machining operations like multi-axis milling, drilling, turning, grinding, etc. For CNC program verification, we use industry standard tools and techniques such as VeriCut and NCVerify to reduce the likelihood of errors, eliminating production time losses.",
        highlight:
          "capability to integrate various processor environments such as Fanuc, Siemens and Fagor. As a part of CNC programming and verification, we offer:",
        points: [
          "Tool path generation",
          "APT programming",
          "CNC program optimization",
          "Post-processor development",
          "Adaptive programming for repair and salvage of components",
          "Virtual machine modeling and simulation",
          "5 Axis Milling",
          "Waterjet Programming",
          "CMM Programming",
          "Process Improvement",
          "How We Can Help",
        ],
      },
    ],
  },
  {
    slug: "cae-development",
    title: "Augmented & Virtual Reality Apps",
    features: [
      {
        id: 1,
        title: "Interfaces",
        icon: "bi bi-diagram-3",
        contentTitle: "Interfaces ",
        points: [
          "Translators for Neutral CAD Formats STEP, VEML, STL, IGES, DXF, Parasolid, Granite, ACIS, JT Open etc",
          "Plug-in Development for 3rd party software Pro/ENGINEER, SolidWorks, CATIA V5/V6, AI, SpaceClaim",
          "Import/Export Translators for ANSYS, MSc Nastran, LS-Dyna, HyperMesh, Abaqus, RADIOSS",
        ],
      },
      {
        id: 2,
        title: "Integration Projects",
        icon: "bi bi-diagram-2",
        contentTitle: "Integration Projects",
        points: [
          "Integrating client software in 3rd party software Pro/ENGINEER, SolidWorks, CATIA V5/V6, AI, SpaceClaim",
        ],
      },
      {
        id: 3,
        title: "Application Development",
        icon: "bi bi-app-indicator",
        contentTitle: "Application Development",
        highlight: "Application Development using CAE Software SDKs.",
        points: [
          "Abaqus: Abaqus Scripting (Python) and Abaqus GUI Tool Kit",
          "Ansys: ANSYS Workbench SDK, ADPL",
          "Hypermesh: Tcl, Tk, C#, C++",
          "LS-Dyna: Tcl, Tk, C#, C++",
        ],
      },
      {
        id: 4,
        title: "Pre/Post Development",
        icon: "bi bi-bar-chart",
        contentTitle: "Pre/Post Development",
        points: [
          "Material models, Contact modelling, Loads and Boundary Conditions, Analysis Setup",
          "Integration with meshing software",
          "Post-Processing & Rendering of Post-processing data and charts",
        ],
      },
      {
        id: 5,
        title: "Analysis Modules",
        icon: "bi bi-kanban",
        contentTitle: "Analysis Modules",
        highlight:
          "Complete Analysis Modules - Structural, Thermal, Fluids, Electro Magnetic, linear/Nonlinear/Transient, Modal Dynamics, Multi-physics.",
        points: [
          "Will be Developed and Customized as per client’s requirements.",
          "Available on licensing basis as well .",
        ],
      },
      {
        id: 6,
        title: "Solvers",
        icon: "bi bi-lightning",
        contentTitle: "Solvers ",
        highlight:
          "Fast Iterative Direct Solvers are available on licensing basis.",
        points: [
          "These solvers are being used by premier Vendors like Ansys Inc, MSC Software, UGS-Siemens, Noran Engineering, Solid works/Cosmos etc",
        ],
      },
      {
        id: 7,
        title: "Design Optimization",
        icon: "bi bi-diagram-3-fill",
        contentTitle: "Design Optimization",
        points: [
          "Plug-in development",
          "Optimization Algorithm Development",
          "Postprocessing RSMS, Surface Plots, Pi Charts etc.",
        ],
      },
      {
        id: 8,
        title: "1D Multi Domain System Simulation",
        icon: "bi bi-diagram-3",
        contentTitle: "1D Multi Domain System Simulation",
        points: ["Components and Libraries for Various Engineering Systems"],
      },
      {
        id: 9,
        title: "Graphics and Visualization",
        icon: "bi bi-eye",
        contentTitle: "Graphics and Visualization",
        points: [
          "GUI to display geometric results",
          "Specifications Results Displays",
          "Q-Slice ",
          "Iso-surface",
          "Controlling particle flow or charged particle trace",
        ],
      },
      {
        id: 10,
        title: "Technologies",
        icon: "bi bi-tools",
        contentTitle: "Core Tools & Libraries",
        highlight: "License Management Tool",
        points: [
          "Languages: C & C++, C#, Visual Basic, Java, .NET, VC++",
          "Graphic Libraries: Qt, Coin3D, OpenGL, OpenCascade, VTK",
          "Parasolid geometry toolkit and geometric algorithms",
          "CATIA CAA V5/V6, Pro/TOOLKIT, SolidWorks API, Tcl/Tk",
        ],
      },

      {
        id: 11,
        title: "Graphics & UI",
        icon: "bi bi-display",
        contentTitle: "Graphics and UI",
        highlight: "License Management Tool",
        points: [
          "UI using Qt, WPF in .NET",
          "3D Graphic rendering using Coin3D/Open Inventor, Open Cascade, OpenGL, VTK",
          "Special graphic feature development",
        ],
      },
      {
        id: 12,
        title: "CAx Data management ",
        icon: "bi bi-cloud-arrow-up",
        contentTitle: "CAx Data management (Big Data and Data Analytics)",
        content:
          "We provide Big Data solution to store, manage and to perform scalable query-driven processing of CAx Data. AI based Data driven CAE platform gives flexibility in carrying out DoE explorations, sensitivity analysis, MOO, MDO and reliability studies more efficiently and economically without any constraints to identify the optimal robust model.",
      },
    ],
  },
  {
    slug: "cax-software-testing",
    title: "CAx Software QA & Testing",
    features: [
      {
        id: 1,
        title: "CAE Software Testing",
        icon: "bi bi-check2-square",
        contentTitle: "CAE Software Testing ",
        points: [
          "Accuracy Testing of CAE software for standard/non-standard engineering problems using exact solutions",
          "Creation of Test plans covering Positive and Negative test scenarios for expected behaviors and updating continuously to reflect changes.",
          "Unit, Component and Integration Testing",
          "Coverage Analysis for identifying Use Cases /Test Cases",
          "Regression Management",
          "Automation Tool Expertise on Rational Visual Test, Silk Test, XRunner, WinRunnerg",
          "Scripting Language Expertise in VB Script, JavaScript, Perl and Shell",
          "Development of API's",
          "Localization Testing",
        ],
      },
      {
        id: 2,
        title: "Supported Platforms",
        icon: "bi bi-laptop",
        contentTitle: "Supported Platforms",
        points: [
          "Windows",
          "Apple Macintosh",
          "Linux",
          "HP-UX",
          "Sun Solaris",
          "IBM AIX",
        ],
      },
      {
        id: 3,
        title: "Why V J Coresoft Testing?",
        icon: "bi bi-star",
        contentTitle: "Why V J Coresoft Testing?",
        points: [
          "Over 500 Man-Years of CAD/CAE Software Testing Experience",
          "Just having an experience on software, will not make a good tester. Over years of experience, our testers developed an eye for detailing, test to break attitude and strong desire for quality. Since we test the product as a first user, we could able to un-earth many bugs that are not seen in development environment.",
          "Virtually we simulate first user environment. Our testing includes, install/uninstall, licensing, report generation, localization testing etc.",
          "Good capabilities in testing Integration and Interfaces with 3rd Party Software.",
          "Our service is not just cost beneficial but it also adds value to development. Our testers have good domain knowledge. Combination of Domain knowledge and testing experience is proving very fruitful.",
          "We support testing activities on all platforms (like Win, Linux, Apple Mac, Unix) - no additional Cost for testing on Unix Platforms.",
          "Along with accuracy testing, we have good expertise in Testing User Scenarios, where real life customer problems are simulated for every Release.",
        ],
      },
      {
        id: 4,
        title: "CAD Testing",
        icon: "bi bi-pencil-square",
        contentTitle: "CAD Testing ",
        highlight:
          "We offer following Testing Domain expertise under one roof.",
        points: [
          "Geometry Transfer",
          "Sketching, Detailing/Drawing",
          "Assembly modeling, 3D ",
          "modeling Features",
          "Expertise on UG, ProE, CATIA, ",
          "SolidWorks, SolidEdge",
          "Mold design and Casting",
          "Sheet metal and welding Design",
        ],
      },
      {
        id: 5,
        title: "CAE Testing",
        subtitle: "Simulation & Analysis",
        icon: "bi bi-bar-chart-line",
        contentTitle: "CAE Testing ",
        highlight:
          "We offer following Testing Domain expertise under one roof.",
        points: [
          "Multi Domain System .",
          "Simulation",
          "Electromagnetic",
          "Structural, Thermal",
          "MultiBody Dynamic",
          "Acoustics",
          "Transmission System Design and Analysis",
          "Design Optimization",
          "Multiphysics including AC/DC, Chemical Engg.",
          "CFD",
          "Crash and Explicit Solvers",
          "Durability",
          "PLM",
          "Injection Molding",
          "XFEM",
          "Manufacturing process simulation: welding, machining, sheet metal forming",
          "Interfaces",
          "Geotechnical",
          "Energy Simulation",
        ],
      },
    ],
  },

  {
    slug: "plm",
    title: "PLM",
    intro:
      "VJCS has multi years of practical expertise in PDM/PLM domain in the areas of Aerospace, Automotive, Heavy Industries etc.",
    features: [
      {
        id: 1,
        title: "Service Offerings",
        icon: "bi bi-briefcase",
        contentTitle: "Service Offerings",
        content:
          "Supporting End-to-end Global PLM systems Commissioning – new implementations, migrations and upgrades, integration, performance tuning; user training and adaptability; customized applications and frameworks; support and sustenance.",
        points: [
          "CAD customization – Using NX Open API, CATIA CAA, Pro/Toolkit, Autodesk Inventor API, SW API, JLink, Pro/Program, etc.",
        ],
      },
      {
        id: 2,
        title: "PLM Tools Expertise",
        icon: "bi bi-tools",
        contentTitle: "PLM Tools Expertise",
        content:
          "Teamcenter, Windchill, Areas, Enovia, eMatrix, SAP PLM, Smarteam, Agile, etc.",
      },
      {
        id: 3,
        title: "Integration Strengths",
        icon: "bi bi-diagram-3",
        contentTitle: "Integration Strengths",
        content:
          "Proven strength in X-PLM, PDX, PLM-XML, Biztalk, TIBCO, Informatica, SOA, BPEL, etc.",
      },
      {
        id: 4,
        title: "PLM Services & Solutions Framework",
        icon: "bi bi-grid",
        contentTitle: "PLM Services & Solutions Framework",
        points: [
          "Business Process Services",
          "Integrations and Visualizations",
          "CAD Customization",
          "Independent QA/Testing",
          "Migration and Upgrades",
          "Support",
        ],
      },
    ],
  },

  {
    slug: "product-design",
    title: "Product Design",
    features: [
      {
        id: 1,
        title: "Concept / Design / Prototype / Production",
        icon: "bi bi-layers",
        contentTitle: "Concept / Design / Prototype / Production",
        points: [
          "Layout",
          "3D Representation of Concept",
          "Packaging",
          "Marker study, benchmark and ergonomics",
          "Product design - conceptual design/ styling",
          "Product Engineering - Electrical, mechanical, hydraulics systems",
          "Prototype Build - Prototype sourcing POC, Alpha 1.4 T, Alpha 1.6 T",
          "Testing & Validation - Third party",
          "Design modification and documentation",
          "Beta prototypes",
          "Production sourcing",
          "Tooling & Fixtures",
          "Estimation for production quantities",
        ],
      },
      {
        id: 2,
        title: "Product Design Support",
        icon: "bi bi-pencil-square",
        contentTitle: " Product Design Support",
        points: [
          "3D Solid & Surface Modeling, Detailing",
          "Piping, Layout Design (for process industries)",
          "GD & T",
          "Assembly Modeling & Detailing",
          "Tolerance Stack-up",
          "Reverse Engineering",
        ],
      },
      {
        id: 3,
        title: "Automation & Customization Expertise",
        icon: "bi bi-cpu",
        contentTitle: "Automation & Customization Expertise",
        points: [
          "CAD: NX, ProE, SolidWorks, Catia (using CAA), Spaceclaim",
          "CAE: Ansa, LSDyna, NX Simulation, Hypermesh, Abaqus, Ansys, MSC Nastran, SolidWorks Simulation",
          "NC Programming Services",
        ],
      },
    ],
  },
  {
    slug: "bim-modelling",
    title: "BIM Modelling Services",
    intro:
      "VJ Coresoft offers customized BIM Modelling services and solutions that suits clients Building Information Modelling (BIM) requirements. Bringing years of experience in various BIM Software we offer BIM modeling services for the Architectural, Structural, MEP, HVAC, Façade, Mechanical and Plumbing & Fire protection domains.",
    features: [
      {
        id: 1,
        title: "BIM Modelling Services",
        icon: "bi bi-building",
        contentTitle: "BIM Modelling Services",
        points: [
          "Architectural BIM Modelling",
          "Structural BIM Modelling",
          "MEP BIM Modelling",
          "Scan to BIM Modelling",
          "BIM Content Creation",
          "Steel Shop Drawings",
          "BIM Clash Detection",
          "Construction Management Software Development and Smart Building",
          "Energy Simulation",
        ],
      },
    ],
  },
  {
    slug: "marine-engineering",
    title: "Marine Engineering",
    intro:
      "We provide high end innovative design & technical solutions to marine and offshore industry. By utilizing our proficiency in engineering & extensive design experience we have successfully delivered range of marine design and engineering services.We offer a complete set of design and consultancy services, starting with the earlier design stages up to production information.\n\nWe are internationally renowned provider of ship design and system solutions for ship & yacht building. For ship design we provide quality basic & detail design engineering services for production. Where the hull structure, outfit and systems are detailed to achieve a cost and time effective production life cycle.\n\nOur Ship Design and Offshore Services Include:",
    features: [
      {
        id: 1,
        title: "Basic Design",
        icon: "bi bi-diagram-3",
        contentTitle: "Basic Design",
        content:
          "VJ Coresoft has developed in-depth capabilities to carry out Basic Design.\nWe utilize our experience gained from the building processes at the various shipyards that we work with and the feedback obtained from vessel operators and equipment manufacturers, to enhance design capability and develop safe and efficient designs for our customers. This has enhanced our design capability in developing safe and efficient vessels for our customers.",
      },
      {
        id: 2,
        title: "Production Design",
        icon: "bi bi-gear",
        contentTitle: "Production Design",
        content:
          "VJCS has substantial experience in this area. We undertake hull design, 3D modelling, creation of production drawings and nesting.",
      },
      {
        id: 3,
        title: " Design & Production Docs",
        icon: "bi bi-journal-text",
        contentTitle: "Detail Design & Production Documentation",
        content:
          "Based on Basic & Production Design, we delivery various outputs by carefully studying shipyard practice and working methodology. VJCS has wide expertise in the area of creation of 3D models and layouts of equipment, structures, machinery parts, piping, cable trays and HVAC systems.",
      },
      {
        id: 4,
        title: "Interior & Exterior Design",
        icon: "bi bi-house",
        contentTitle: "Interior and Exterior Design",
        content:
          "VJCS provides Interior & Exterior Design Services for Accommodation Areas, Inner & Outer Deck Areas, for Ship Building Industry.",
      },
      {
        id: 5,
        title: "3D Modeling",
        icon: "bi bi-box",
        contentTitle: "Comprehensive 3D Modeling Services",
        content:
          "VJCS provides Interior & Exterior 3D Visualizations Services, for Shipbuilding Industry. The views indicate even the intricate level of detailing for exteriors as well as interiors.",
      },
      {
        id: 6,
        title: "CAE Analysis",
        icon: "bi bi-graph-up",
        contentTitle: "Robust CAE Analysis for Ships",
        content:
          "CAE tools are being used, to analyse the robustness and performance of Ship components and assemblies. The term encompasses simulation, validation, and optimization of products and manufacturing tools.",
        highlight: "Our services includes -",
        points: [
          "Local Structure Strength Analysis",
          "Global Ship Vibration Analysis",
          "Global Ship Structure Strength Analysis",
          "Thermal Analysis",
          "Local Structure Vibration Analysis",
          "Computational Fluid Dynamics (CFD) Analysis",
        ],
      },
    ],
  },
  {
    slug: "virtual-manufacturing",
    title: "Virtual Manufacturing",
    features: [
      {
        id: 2,
        title: "Virtual Manufacturing Services",
        icon: "bi bi-cpu",
        contentTitle: "Virtual Manufacturing Services",
        highlight: "Our Virtual Manufacturing Services includes",
        points: [
          "Virtual commissioning, Validating PLC code and HMI",
          "Plant simulation",
          "Human simulation and ergonomics",
          "Robot simulation, through validation & offline programming (OLP) for various applications",
          "Machining simulation, via CNC Programming, tool and path generation, Skill Sets – C,C++, Java,Tecnomatix, .Net Api, XML",
        ],
      },
    ],
  },
  {
    slug: "industrial-iot",
    title: "Industrial Internet of Things",
    features: [
      {
        id: 2,
        title: "Industrial Internet of Things",
        icon: "bi bi-cpu",
        contentTitle: "Industrial Internet of Things",
        content:
          "VJCS end-to-end IoT solutions empower smart industries, smart living and smart enterprises and deliver connected experiences by connecting assets, operations/logistics, and services. We focus predominantly on the segments mentioned below. we bring expertise with platforms like Hitachi Lumada, AWS IoT, and Azure IoT, further combined with technologies like Augmented Reality (AR), and Artificial Intelligence (AI) to open new possibilities and help generate business value for enterprises in innovative ways.",
        points: [
          "Smart manufacturing",
          "Smart facilities",
          "Smart equipment",
          "Apps for connected cars",
        ],
      },
    ],
  },

  {
    slug: "ai-solutions",
    title: "AI Solutions & Services",
    intro:
      "At VJ Coresoft, we’re working with forward thinking businesses in developing solutions for the workforce of the future using AI and combining it with technologies such Augmented Reality (AR), Artificial Intelligence (AI), Cloud, Analytics and Industrial IoT to help businesses successfully undergo digital transformation initiatives.",
    features: [
      {
        id: 1,
        title: "Manufacturing",
        icon: "bi bi-gear",
        contentTitle: "Manufacturing",
        points: [
          "Predictive maintenance",
          "Visual quality control",
          "Demand forecasting",
          "Early-warning systems",
        ],
        image: "/assets/img/misc/misc-square-13.webp",
      },
      {
        id: 2,
        title: "Automotive",
        icon: "bi bi-car-front",
        contentTitle: "Automotive ",
        points: [
          "Image recognition, speech recognition, gesture recognition",
          "Predictive maintenance",
          "AI-based in-vehicle infotainment systems",
          "AI Driver Assist and autonomous vehicles",
        ],
      },
      {
        id: 3,
        title: "Sports Fitness",
        subtitle: "AI in Sports & Fitness",
        icon: "bi bi-activity",
        contentTitle: "Sports Fitness ",
        points: [
          "Computer vision and gesture recognition",
          "Virtual fitting rooms and mirrors using AI and AR",
          "In-store assistance",
        ],
      },
    ],
  },

  {
    slug: "enterprise-web-mobile",
    title: "Enterprise Web & Mobile",
    features: [
      {
        id: 1,
        title: "Optimize Factory Maintenance",
        icon: "bi bi-wrench-adjustable",
        contentTitle: "Optimize Factory Maintenance",
        content:
          "With enterprise mobility solutions for operations, Client can access to various information related to workshop safety, information & undergoing operations on the go so that client business can stay incident free & helps to achieve operational efficiency.",
        points: [
          "Access to Factory Information System on the go with a mobile app",
          "Access to Quality Management Systems on the go with a mobile app",
          "Get an access to workshop equipment safety & security with enterprise mobile apps for manufacturing",
          "Production planning & scheduling on the go",
        ],
      },
      {
        id: 2,
        title: "Monitor Equipment Health in Real-time",
        icon: "bi bi-speedometer2",
        contentTitle: "Monitor Equipment Health in Real-time",
        content:
          "With VJCS enterprise mobility solutions for manufacturing, track productionequipment efficiency & monitor their health on mobile devices; to conduct preemptive maintenance checks at regular intervals.",
        points: [
          "Access equipment related critical data of factory & take action on it on the go from the mobile app",
          "Prevent material wastage and reduce input overheads of manufacturing costs by accessing its data from a mobile app .",
          "Eliminate redundant activities and improve internal collaboration between SBUs with an enterprise mobile app",
          "Maintain the quality of your products by conducting scheduled equipment maintenance",
        ],
      },
      {
        id: 3,
        title: "Reduce Time-to-Market for New Products",
        icon: "bi bi-truck",
        contentTitle: "Reduce Time-to-Market for New Products",
        content:
          "Supply Chain enterprise mobility solutions to overcome the issues of production planning, material tracking & dispatch",
        points: [
          "Get a high-level view of material available to plan the uninterrupted production line",
          "High employee visibility on shop floor; enabling smarter resource management",
          "Smooth flow of data & information to all the stakeholders such as customers, vendors & suppliers",
          "Operational efficiency through fleet management",
        ],
      },
    ],
  },
  {
    slug: "ar-vr-apps",
    title: "Augmented & Virtual Reality Apps",
    intro:
      "At VJCS, we use a well-balanced set of technologies to create incredible solutions that will make you stand out from the crowd. Our team focuses on custom solutions for the top virtual reality platforms, including HTC Vive and Windows Mixed Reality. We also offer state-of-the-art game development, through platforms, such as Unity and Unreal Engine. Our extensive experience is put to good use, ensuring that our clients have the highest-value technical solution for their particular business challenge.",
    features: [
      {
        id: 1,
        title: "3D Modeling",
        icon: "bi bi-cube",
        contentTitle: "3D Modeling",
        points: [
          "We provide the absolute best VR experience for your branding and corporate vision. We create content, animations and special effects for interactive VR/AR projects.",
        ],
      },
      {
        id: 2,
        title: "AR App Development",
        icon: "bi bi-phone-vibrate",
        contentTitle: "AR App Development",
        points: [
          "We build AR solutions for retail, e-commerce, healthcare and other industries. We also combine AR with other technologies to create an even more captivating experience.",
        ],
      },
      {
        id: 3,
        title: "VR App Development",
        icon: "bi bi-vr",
        contentTitle: "VR App Development",
        points: [
          "Our in-house team creates mind-blowing VR experiences across multiple platforms and industries.",
        ],
      },
      {
        id: 4,
        title: "MR App Development",
        icon: "bi bi-collection-play",
        contentTitle: "MR App Development",
        points: [
          "With the help of Unity, our developers build mixed reality interfaces to manage business activities and facilitate productivity from anywhere.",
        ],
      },
      {
        id: 5,
        title: "Multi-Platform Solutions",
        icon: "bi bi-hdd-network",
        contentTitle: "Multi-Platform Solutions",
        points: [
          "We focus on custom solutions for a wide range of top virtual reality headsets and platforms, including HTC Vive, Windows Mixed Reality and more.",
        ],
      },
      {
        id: 6,
        title: "Geo-location AR Apps",
        icon: "bi bi-geo-alt",
        contentTitle: "Geo-location AR Apps",
        points: [
          "Get ahead of the competition with a new VR app using location-based touch.",
        ],
      },
      {
        id: 7,
        title: "360° Video",
        icon: "bi bi-camera-reels",
        contentTitle: "360° Video",
        points: [
          "Our team develops custom apps on every major platform delivering seamless 360° video experiences to our customers.",
        ],
      },
      {
        id: 8,
        title: "Support & Maintenance",
        icon: "bi bi-life-preserver",
        contentTitle: "Ongoing AR/VR Support & Maintenance",
        points: [
          "Our team will deliver product stability and optimized business processes by providing ongoing support and maintenance.",
        ],
      },
    ],
  },
];
